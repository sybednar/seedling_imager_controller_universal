...
def force_centered_scaler_crop() -> None:
    """
    Force the scaler crop to the full sensor area so preview and still streams
    share the same centered field of view (no digital zoom/crop drift).
    Safe to call after picam.start().
    """
    try:
        props = picam.camera_properties
        pa_w, pa_h = props.get("PixelArraySize", (0, 0))
        if pa_w and pa_h:
            picam.set_controls({"ScalerCrop": (0, 0, int(pa_w), int(pa_h))})
            print(f"[camera] ScalerCrop set to full array {pa_w}x{pa_h}", flush=True)
    except Exception as e:
        print(f"force_centered_scaler_crop error: {e}", flush=True)
...
def get_frame() -> QImage:
    ...
    try:
        with _cam_lock:
            arr = picam.capture_array("lores")  # 1280x720; fast preview
    except Exception as e:
...
# --- NEW: Main-stream preview (downscaled) for alignment-critical snapshots ---
def get_frame_from_main_for_preview(max_w: int = 1280, max_h: int = 720) -> QImage:
    """
    Return a QImage built from the *main* stream (full-res) but downscaled
    to fit within (max_w, max_h). Use this for monitor snapshots so the FOV
    matches the saved stills exactly.
    """
    try:
        with _cam_lock:
            arr = picam.capture_array("main")  # full-res frame
    except Exception as e:
        print(f"get_frame_from_main_for_preview error: {e}", flush=True)
        return QImage()

    rgb = _to_rgb(arr)  # HxWx3 uint8
    h, w = rgb.shape[:2]
    # Compute a fit that preserves aspect (no crop), bounded by max_w/max_h
