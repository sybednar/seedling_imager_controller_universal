# experiment_runner.py — acquisition loop with robust full re-homing
...
REHOME_EVERY_N = 1
FIRST_PLATE_WARMUP_S = 3.0
AE_GATE_MAX_WAIT_S = 3.0
AE_GATE_POLL_S = 0.10
AE_GATE_GAIN_TOL = 0.05
AE_GATE_STABLE_READS = 5
...
class ExperimentRunner(QThread):
    ...
    def __init__(self, selected_plates, duration_days, frequency_minutes,
                 illumination_mode, led_control_fn, perform_homing: bool = True, parent=None):
    ...
    def _log(self, msg): self.status_signal.emit(msg)
    ...
    def _open_csv(self):
        try:
            ...
            self.csv_writer.writerow([
                "timestamp_iso", "cycle_index", "plate", "illumination", "image_path",
                "width_px", "height_px", "file_size_bytes",
                "AeEnable", "ExposureTime_us", "AnalogueGain", "AwbEnable",
                "SettledExposureTime_us", "SettledAnalogueGain",
                "LensPosition", "AfState", "FocusFoM",
            ])
    ...
    def _autofocus_with_retry(self, threshold: float = 500.0, timeout_s: float = 3.0, poll_s: float = 0.2):
        best_fom, best_md, attempts = None, {}, 0
        def _poll():
            ...
            while (time.time() - t0) < timeout_s and not self._abort:
                ...
                try:
                    ...
                    fv = None
                if fv is not None:
                    if (best_fom is None) or (fv > best_fom):
                        best_fom = fv; best_md = md
                    if fv >= threshold:
        ...
        try: camera.set_af_mode(1)
        except Exception as e: self._log(f"AF mode set warning (ignored): {e}")
        attempts = 1
        try: camera.trigger_autofocus()
        except Exception as e: self._log(f"AF trigger warning (ignored): {e}")
        best_fom, best_md, ok = _poll()
        if ok or self._abort: return (best_fom, best_md, attempts)
        attempts = 2
        self._log(f"Plate focus retry: FocusFoM<{threshold} after {timeout_s:.1f}s; retrying autofocus once...")
        try: camera.trigger_autofocus()
        except Exception as e: self._log(f"AF retry trigger warning (ignored): {e}")
        best_fom, best_md, ok2 = _poll()
    ...
    def _rehome_at_cycle_boundary(self):
        if not REHOME_EVERY_N or (self.cycle_count % int(REHOME_EVERY_N) != 0):
            return
        try: motor_control.driver_enable()
    ...
    def run(self):
        ...
        # Global pre-warm (especially IR)
        try:
            if self.illumination_mode == "Infrared" and self.led_control_fn:
        ...
        try:
            while datetime.now() < end_time and not self._abort:
                ...
                for plate_idx in range(1, 7):
                    ...
                    if best_fom is None:
                        ...
                        self._log(f"Plate #{plate_idx}: FocusFoM best={best_fom:.0f} (threshold=500, attempts={attempts}), locking focus.")
                    try: camera.set_af_mode(0)
                    except Exception as e: self._log(f"AF lock warning (ignored): {e}")
                    ...
                    if plate_idx in self.selected_plates:
                        ...
                        if saved:
                            ...
                            try: file_size = Path(img_path).stat().st_size
                            ...
                            if self.csv_writer:
                                self.csv_writer.writerow([
                                    ...
                                    self.cycle_count, plate_idx, self.illumination_mode,
                                    img_path, width, height, file_size,
                                    AeEnable, ExposureTime, AnalogueGain, AwbEnable,
                                    settled_exp, settled_gain, lens_pos, af_state, focus_fom,
                                ])
            ...
            try: camera.apply_settings(self.cam_settings)
            except Exception: pass
